...
这是一个使用 C++17 开发的本地日志分析工具。当前已经支持按批读取普通文件、解析固定格式日志、筛选结构化日志，以及将分析结果写入文本输出流；线程调度、命令行和完整扫描流水线仍在开发中。
...
- `ConsoleReportSink` 将 Finding 输出为 `文件:行号 [级别] 正文`，并在结束时输出扫描统计。
...
summary: batches=<批次数> records=<读取记录数> analyzed=<分析记录数> findings=<匹配数>
...
默认构建会运行公共 API、文件批量读取、固定格式解析、筛选分析和报告输出测试。
